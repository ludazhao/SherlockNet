...
def build_vocab(imgs, params):
  ...
  #bad_words = [w for w,n in counts.iteritems() if n <= count_thr or not d.check(w)]
  vocab = [w for w,n in counts.iteritems() if n > count_thr and d.check(w) and not hasNumbers(w)]
  #print "vocab first 50:"
  #print vocab[:50]
  ...
  for img in imgs:
    img['final_captions'] = []
    for txt in img['processed_tokens']:
      caption = [w if counts.get(w,0) > count_thr and d.check(w) and not hasNumbers(w) else 'UNK' for w in txt]
      if (len(caption) == 0):
        counter+=1
        caption = ['UNK']
      img['final_captions'].append(caption)
  print "number of empty captions,", counter
  return vocab
