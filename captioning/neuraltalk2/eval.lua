...
cmd:option('-output_folder', '', 'output file path of vis')
...
local function eval_split(split, evalopt)
  ...
  while true do
    ...
    for k=1,#sents do
      local entry = {image_id = data.infos[k].id, caption = sents[k], fn = data.infos[k].file_path}
      ...
      --if opt.dump_images == 1 then
        ...
        -- local cmd = 'cp "' .. path.join(opt.image_root, data.infos[k].file_path) .. '" vis_' .. opt.output_folder .. '/imgs/img' .. #predictions .. '.jpg' -- bit gross
        --print(cmd)
...
if opt.dump_json == 1 then
  -- dump the json
  utils.write_json('vis/vis_json/vis_' .. opt.output_folder .. '.json', split_predictions)
end
